...
# 需要编译的项目地址 
resource_directory = r'/home/jskj/Desktop/tmp/MXT_QC_TopTally' 
# 编译后存放so/pyd文件的项目地址
build_directory = r'/home/jskj/Desktop/tmp/package' 
# 不编译的文件，例如程序入口
...
def copy_project():
    ...
    将当前项目复制到目标编译文件夹
    """ 
    if os.path.exists(build_directory): # 如果目标路径存在就先删除
    ...
    shutil.copytree(resource_directory, build_directory, 
...
def load_project_resource(dir_path, extensions): 
    ...
    for file_name in os.listdir(dir_path):
        ...
        elif file_name.endswith(resource_suffix) and file_name not in exclude_build: # 加载所有需要编译的资源文件
...
def remove_resource(dir_path):
    ...
    for file_name in os.listdir(dir_path):
        ...
        elif file_name.endswith(('.py', '.c', '.html')) \
...
def build(extensions):
    ...
    for file_path in extensions:
        setup(
            ...
            # options={
            #     "build": {
            #         "build_lib": '/',#将编译好的pyd文件放在python文件目录下
            #     }
            # },
        )
...
if __name__ == '__main__':
    assert os.path.abspath(resource_directory) != os.path.abspath(build_directory), '为了防止出错，项目文件夹不能和编译文件夹一致'
    # 复制项目到编译文件夹
    copy_project()
    # 加载所有可以编译的资源文件
    ...
    # 删除py文件和pyx文件
    remove_resource(build_directory)
